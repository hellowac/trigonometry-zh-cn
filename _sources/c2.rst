...
在初等几何中，你学到三角形的面积是底乘以高的一半。现在我们将运用这一原理，结合一些三角学知识，推导出更多已知三角形各部分面积的公式。

*案例 1：两条边和夹角。*

假设我们有一个三角形 :math:`\triangle ABC`，其中 $A$ 可以是锐角、直角或钝角，如图 :ref:`2.4.1 <fig:areacase1>` 所示。假设 $A$、$b$ 和 $c$ 已知。
...
在每种情况下，我们从顶点 $C$ 到 :math:`\overline{AB}` 画一条高为 $h$ 的线，因此
面积（我们用字母 $K$ 表示）等于 :math:`K = \frac{1}{2}hc`。但我们看到
:math:`h = b\sin\;A` 在每个三角形中（因为图 :ref:`2.4.1 <fig:areacase1>` (b) 中 :math:`\;h=b` 且 :math:`\sin\;A = \sin\;90^\circ = 1`，而图 :ref:`2.4.1 <fig:areacase1>` (c) 中 :math:`\;h = b\sin\;(180^\circ - A) = b\sin\;A`）。因此，我们得到以下公式：
...
上述 :math:`\triangle ABC` 的面积公式是根据已知部分 $A$、$b$ 和 $c$ 计算的。对于角 $B$ 和 $C$，类似的推导可得：
...
请注意，高 $h$ 在这些公式中并未明确出现，尽管它隐含在其中。这些公式的优点在于，它们以三角形的部分表示，无需单独计算 $h$。
...
.. admonition:: Example 2.13
    ...
    已知 :math:`A = 33^\circ`、$b = 5$ 和 $c = 7$，求三角形 :math:`\triangle ABC` 的面积。

    **解答：** 使用公式 :eq:`2.23 <eqn:areacase1a>`，面积 $K$ 如下：
    ...
    *情况 2：三个角和任意边。*
    ...
    假设我们有一个三角形 :math:`\triangle ABC`，其中一条边（例如 $a$）和所有三个角都是已知的。 [7]_ 
    ...
    因此将其代入公式 :eq:`2.24 <eqn:areacase1b>` 可得：
    ...
    .. [7] 注意，这相当于只知道 \emph{两个} 角和一条边（为什么？）。

           Note that this is equivalent to knowing just \emph{two} angles and a side (why?).
...
.. admonition:: Example 2.14
    ...
    已知 :math:`A = 115^\circ`、:math:`B=25^\circ`、:math:`C=40^\circ` 和 $a = 12$，求三角形 :math:`\triangle ABC` 的面积。

    **解答：** 使用公式 :eq:`2.26 <eqn:areacase2a>`，面积 $K$ 计算如下：
...
*情况三：三条边。* 假设我们有一个三角形 :math:`\triangle ABC`，其三条边均已知。那么 *海伦公式* [8]_ 可以计算出三角形的面积：

.. [8] 源于古希腊工程师兼数学家亚历山大的海伦（约公元 10-70 年）。

    Due to the ancient Greek engineer and mathematician Heron of Alexandria (c. 10-70 A.D.).

    **海伦公式**：对于边长为 $a$、$b$ 和 $c$ 的三角形 :math:`\triangle ABC`，令 :math:`s = \frac{1}{2}\,(a+b+c)` （即 :math:`2s = a+b+c` 是三角形的周长）。那么三角形的面积 $K$ 为
...
为了证明这一点，首先要记住，面积 $K$ 等于底乘以高的一半。以 $c$ 为底，以高 $h$ 为高，如前图 :ref:`2.4.1 <fig:areacase1>` 所示，可得 :math:`K = \frac{1}{2}hc`。两边取平方可得
...
在图 :ref:`2.4.2 <fig:heron>` 中，令 $D$ 为高与 :math:`\overline{AB}` （或其延长线）的交点。
...
根据勾股定理，我们得到 :math:`\;h^2 = b^2 - (AD)^2`。在图 :ref:`2.4.2 <fig:heron>` (a) 中，我们看到 :math:`\;AD = b\;\cos\;A`。在图 :ref:`2.4.2 <fig:heron>` (b) 中，我们看到 :math:`\;AD = b\;\cos\;(180^\circ - A) = -b\cos\;A`。因此，无论哪种情况，我们都有 :math:`\;(AD)^2 = b^2 \;(\cos\;A)^2`，因此
...
（注意，当 :math:`A=90^\circ` 时，上述等式也成立，因为 :math:`\cos\;90^\circ =0` 且 $h=b$）。
因此，将方程 :eq:`2.31 <eqn:heronproof2>` 代入方程 :eq:`2.30 <eqn:heronproof1>`，可得
...
因此，将这些表达式代入方程 :eq:`2.32 <eqn:heronproof3>`，我们得到
...
由于我们定义了 :math:`s = \frac{1}{2}\,(a+b+c)`，因此
...
.. admonition:: Example 2.15
    ...
    给定 $a=5$、$b=4$ 和 $c = 7$，求三角形 :math:`\triangle ABC` 的面积。

    **解答：** 使用海伦公式，其中 :math:`s = \frac{1}{2}\,(a+b+c) = \frac{1}{2}\,(5+4+7) = 8`，面积 $K$ 为：
...
海伦公式在理论方面很有用（例如，在推导其他公式时）。然而，它不太适合用计算器计算，在计算“极端”三角形时会表现出所谓的“数值不稳定性”，如下例所示。
...
.. admonition:: Example 2.16
    ...
    给定 $a=1000000$、$b=999999.9999979$ 和 $c = 0.0000029$，求三角形 :math:`\triangle ABC` 的面积。

    **解答：** 要使用海伦公式，我们需要计算 :math:`s = \frac{1}{2}\,(a+b+c)`。注意，$a+b+c$ 的实际值为 $2000000.0000008$，有 $14$ 位数字。大多数计算器内部可以存储 $12$-$14$ 位数字（即使显示的位数更少），因此可能会将 $a+b+c$ 的值四舍五入为 $2000000$。当我们将 $a+b+c$ 的四舍五入值除以 $2$ 得到 $s$ 时，某些计算器（例如 TI-83 Plus）会给出向下舍入的值 $1000000$。

    这是一个问题，因为 $a=1000000$，所以我们会得到 $s-a=0$，导致海伦公式计算出的三角形面积为 $0$！这确实是 TI-83 Plus 返回的错误答案。其他计算器可能会给出其他不准确的答案，具体取决于它们内部存储值的方式。实际面积（精确到小数点后 15 位）为 $K = 0.99999999999895$，即它基本上为 $1$。
...
上面的例子展示了 *浮点运算* 的复杂性。[9]_ 幸运的是，当三条边已知时，有一个更好的公式 [10]_ 来计算三角形的面积：

对于边长为 :math:`\triangle ABC` 的三角形 :math:`a \ge b \ge c`，其面积为：
...
.. [9] 即使在现代计算机上，这仍然是一个问题。 D. Goldberg 的文章《每个计算机科学家都应该知道的浮点运算》对这一重要主题进行了精彩的概述，网址为 http://docs.oracle.com/cd/E19957-01/806-3568/ncg_goldberg.html

   This is an issue even on modern computers. There is an excellent overview of this important subject in the article *What Every Computer Scientist Should Know About Floating-Point Arithmetic* by D. Goldberg, available at http://docs.oracle.com/cd/E19957-01/806-3568/ncg_goldberg.html

.. [10] 感谢 W. Kahan：http://www.eecs.berkeley.edu/~wkahan/Triangle.pdf

   Due to W. Kahan: http://www.eecs.berkeley.edu/~wkahan/Triangle.pdf

要使用此公式，请将边的名称排序，使得 :math:`a \ge b \ge c`。然后， *按照公式中出现的顺序（包括括号的使用）对平方根内的运算进行操作* 。然后，取平方根并除以 4。对于示例 :ref:`2.16 <exmp:heronfail>` 中的三角形，上述公式在与海伦公式不符的同一台 TI-83 Plus 计算器上，给出的答案恰好是 $K = 1$。这个公式的神奇之处在于，它只是海伦公式的重写版！括号的使用是为了确保数值稳定性，强制运算顺序正确。

另一个公式 [11]_ 给出了三角形三条边的面积，如下所示：

.. [11] 源自中国数学家邱九韶（约 1202-1261 年）。

   Due to the Chinese mathematician Qiu Jiushao (ca. 1202-1261).

对于边长为 :math:`a \ge b \ge c` 的三角形 :math:`\triangle ABC`，其面积为：
...
对于示例 :ref:`2.16 <exmp:heronfail>` 中的三角形，在使用海伦公式计算失败的同一台 TI-83 Plus 计算器上，上述公式的计算结果恰好为 $K = 1$。
...
练习 1-6，求三角形 :math:`\triangle ABC` 的面积。
...
7. 查找下图 :ref:`2.4.3 <fig:areaquad>` 中四边形的面积。
...
.. figure:: ./img/f2.4.3.png
    ...
    **图 2.4.3 练习** :ref:`7 <exer:areaquad>`
...
.. figure:: ./img/f2.4.4.png
    ...
    **图 2.4.4 练习** :ref:`8 <exer:areaquaddiag>`

.. _exer:areaquaddiag:

8. 设 $ABCD$ 为完全包含其两条对角线的四边形，如图 :ref:`2.4.4 所示<fig:areaquaddiag>` 如上所述。证明 $ABCD$ 的面积 $K$ 等于其对角线与它们所夹角的正弦值乘积的一半，即 :math:`K = \frac{1}{2}\,AC\,\cdot\,BD\;\sin\;\theta\;`。

9. 根据公式 :eq:`2.26 <eqn:areacase2a>` 推导出三角形 :math:`\triangle ABC` 的面积公式：
...
与海伦公式等价。 （*提示：在海伦公式中，将* $s$ 替换为 :math:`\frac{1}{2}(a+b+c)`。）

.. _exmp:jiushao:

11. 证明三角形面积公式 :eq:`2.34 <eqn:jiushao>` 与海伦公式等价。（*提示：将表达式分解为平方根。*）
12. 求出例题 :ref:`2.16 <exmp:heronfail>` 中的角 $A$，然后使用公式 :eq:`2.23 <eqn:areacase1a>` 求面积。这样算对了吗？
...
回想一下正弦定理，任何三角形 :math:`\triangle ABC` 都有一个边与对角正弦的公比，即

.. math::

    \frac{a}{\sin\;A} ~=~ \frac{b}{\sin\;B} ~=~ \frac{c}{\sin\;C} ~。

这个公比具有几何意义：它是 :math:`\triangle ABC` 内接的唯一圆的直径（即半径的两倍），这个圆称为三角形的 **外接圆** 。在证明这一点之前，我们需要复习一些初等几何知识。

圆的 **圆心角** 是指顶点为圆心 $O$，边（称为 **半径** ）是从 $O$ 到圆上两点的线段的角。在图 :ref:`2.5.1 <fig:angletypes>` (a) 中，:math:`\angle\,O` 是圆心角，我们说它 *截取圆弧* :math:`\stackrel\frown{BC}`。
...
圆的 **内接角** 是指顶点为圆上点 $A$，边（称为 **弦** ）是从 $A$ 到圆上另外两点的线段的角。圆。在图 :ref:`2.5.1 <fig:angletypes>` (b) 中，:math:`\angle\,A` 是一个与圆弧 :math:`\stackrel\frown{BC}` 截取的内接角。我们在此无需证明 [12]_ 给出内接角与圆心角之间的一个实用关系：

.. _thm:centralangle:

**定理 2.4.** 如果内接角 :math:`\angle\,A` 和圆心角 :math:`\angle\,O` 截取同一段圆弧，则 :math:`\angle\,A = \frac{1}{2}\,\angle\,O\,`。因此，截取同一段圆弧的内接角相等。

.. [12] 证明请参见 *R.A. Avery* 著《平面几何》第 210-211 页，波士顿：Allyn 出版社\& Bacon，1950 年。
   
   For a proof, see pp. 210-211 in *R.A. Avery*, *Plane Geometry*, Boston: Allyn \& Bacon, 1950.

图 :ref:`2.5.1 <fig:angletypes>` (c) 显示了两个内接角 :math:`\angle\,A` 和 :math:`\angle\,D` ，它们与圆心角 :math:`\angle\,O` 截取同一条弧 :math:`\stackrel\frown{BC}`，因此 :math:`\angle\,A = \angle\,D = \frac{1}{2}\,\angle\,O` （所以 :math:`\;\angle\,O = 2\,\angle\,A = 2\,\angle\,D\,` ）。

现在我们将证明关于正弦定理中公比的断言：

.. _thm:circumscribedradius:

**定理 2.5.** 对于任意三角形 :math:`\triangle ABC`，其外接圆半径 $R$ 为：
...
（注意：对于直径为 $1$ 的圆，这意味着 :math:`a=\sin\;A`， :math:`b=\sin\;B`， :math:`c=\sin\;C`。）

为了证明这一点，设 $O$ 为三角形 :math:`\triangle ABC` 的外接圆的圆心。则 $O$ 可以在三角形内部、外部或上面，如图所示参见下面的 :ref:`2.5.2 <fig:circumscribedradius>`。在前两种情况下，从 $O$ 到 :math:`\overline{AB}` 点 $D$ 处画一条垂直线段。
...
.. figure:: ./img/f2.5.2.png
    ...
    **图 2.5.2** :math:`\triangle ABC` **的外接圆**

半径 :math:`\overline{OA}` 和 :math:`\overline{OB}` 的长度相等，因此 :math:`\triangle\,AOB` 是一个
等腰三角形。因此，根据初等几何知识，我们知道 :math:`\overline{OD}` 平分了
角 :math:`\angle\,AOB` 和边 :math:`\overline{AB}`。因此 :math:`\angle\,AOD = \frac{1}{2}\,\angle\,AOB`
且 :math:`AD = \frac{c}{2}`。但由于内接角 :math:`\angle\,ACB` 和圆心角
:math:`\angle\,AOB` 截取同一段弧 :math:`\stackrel\frown{AB}`，根据定理 :ref:`2.4 <thm:centralangle>` 可知，
:math:`\angle\,ACB = \frac{1}{2}\,\angle\,AOB`。因此， :math:`\angle\,ACB = \angle\,AOD`。又由于 :math:`C = \angle\,ACB`，我们有
...
因此，根据正弦定理，如果 $O$ 在 :math:`\triangle ABC` 的内部或外部，则结果成立。

现在假设 $O$ 位于 :math:`\triangle ABC` 上，比如在 :math:`\overline{AB}` 边上，如图 :ref:`2.5.2 <fig:circumscribedradius>` (c) 所示。则 :math:`\overline{AB}` 是圆的直径，因此根据泰勒斯定理， :math:`C =90^\circ` 。因此，:math:`\sin\;C = 1`，从而 :math:`2\,R = AB = c = \frac{c}{1} = \frac{c}{\sin\;C}\;`，结果同样符合正弦定律。[qed]

.. toggle::

    Recall from the Law of Sines that any triangle :math:`\triangle ABC` has a common ratio of sides to sines of opposite angles, namely
...
.. admonition:: Example 2.17
    ...
    求三角形 :math:`\triangle ABC` 的外接圆半径 $R$，其边长为 $a=3$、$b=4$ 和 $c=5$。

    **解答：** 我们知道 :math:`\triangle ABC` 是直角三角形。因此，从图 :ref:`2.5.3 <fig:circum345>` 可以看出，:math:`\sin\;A = 3/5`。因此，
    ...
    注意，由于 $R =2.5$，圆的直径为 $5$，与 $AB$ 相同。因此，:math:`\overline{AB}` 必定是圆的直径，所以圆心 $O$ 是 :math:`\overline{AB}` 的中点。
...
.. _cor:circumscribedright:
    ...
    **推论 2.6.** 对于任何直角三角形，斜边是外接圆的直径，即圆心是斜边的中点。
...
对于上例中的直角三角形，外接圆很容易画；它的圆心可以通过沿 :math:`\overline{AB}` 线测量 $2.5$ 个单位到 $A$ 处来找到。

对于锐角三角形和钝角三角形，我们需要不同的方法，因为对于锐角三角形，外接圆的圆心位于三角形内部，而对于钝角三角形，外接圆的圆心位于三角形外部。从定理 :ref:`2.5 <thm:circumscribedradius>` 的证明中注意到，圆心 $O$ 位于其中一条边 (:math:`\overline{AB}`) 的垂直平分线上。对于其他边，类似的论证表明 $O$ 位于这些边的垂直平分线上：
...
.. _cor:circumscribedcenter:
    ...
    **推论 2.7.** 对于任何三角形，其外接圆的圆心是边垂直平分线的交点。
...
回想一下几何学中如何创建线段的垂直平分线：在每个端点用圆规画一条半径相同的圆弧。选择足够大的半径，使两条圆弧相交于两点，如图 :ref:`2.5.4 <fig:perpbisect>` 所示。过这两点的线就是线段的垂直平分线。对于三角形的外接圆，你只需要两条边的垂直平分线；它们的交点就是圆心。
...
.. admonition:: Example 2.18
    ...
    求 2.2 节中例题 :ref:`2.6 <exmp:case4cosine>` 中三角形 :math:`\triangle ABC` 的外接圆半径 $R$：$a = 2$，$b = 3$，$c = 4$。然后画出三角形和圆。

    **解答：** 在例题 :ref:`2.6 <exmp:case4cosine>` 中，我们发现 :math:`A=28.9^\circ`，因此 :math:`2\,R = \frac{a}{\sin\;A} = \frac{2}{\sin\;28.9^\circ} = 4.14`，因此 :math:`\boxed{R = 2.07}\;`。在图 :ref:`2.5.5 <fig:circum234>` (a) 中，我们展示了如何绘制 :math:`\triangle ABC`：用尺子画出最长边 :math:`\overline{AB}`，其长度为 $c=4$，然后用圆规分别以 $A$ 和 $B$ 为圆心，分别画出半径为 $3$ 和 $2$ 的圆弧。圆弧的交点为顶点 $C$。
    ...
    在图 :ref:`2.5.5 <fig:circum234>` (b) 中，我们展示了如何绘制外接圆：画出 :math:`\overline{AB}` 和 :math:`\overline{AC}` 的垂直平分线；它们的交点就是圆心 $O$。用圆规画出以 $O$ 为圆心、过 $A$ 的圆。
...
定理 :ref:`2.5 <thm:circumscribedradius>` 可用于推导三角形面积的另一个公式：

.. _thm:areacircumradius:

    **定理 2.8.** 对于三角形 :math:`\triangle ABC`，设 $K$ 为其面积，$R$ 为其外接圆的半径。则
    ...
    .. math::
        ...
        K ~=~ \frac{abc}{4\,R} \quad ( \text{因此 }\; R ~=~ \frac{abc}{4\,K} ~) ~。

为了证明这一点，请注意，根据定理 :ref:`2.5 <thm:circumscribedradius>`，我们有
...
将这些表达式代入 2.4 节中关于面积 $K$ 的公式 :eq:`2.26 <eqn:areacase2a>` 中：
...
结合定理 :ref:`2.8 <thm:areacircumradius>` 和海伦三角形面积公式，可得：

.. _cor:circumradiusheron:

    **定理 2.9.** 对于三角形 :math:`\triangle ABC`，令 :math:`s = \frac{1}{2}(a+b+c)`。则其外接圆的半径 $R$ 为
...
除了外接圆之外，每个三角形都有一个 **内切圆** ，即三角形的三条边与其相切的圆，如图 :ref:`2.5.6 <fig:inscribed>` 所示。
...
.. figure:: ./img/f2.5.6.png
    ...
    **图 2.5.6** :math:`\triangle ABC` **的内切圆**

设 $r$ 为内切圆的半径，$D$、$E$ 和 $F$ 分别为 :math:`\overline{AB}`、:math:`\overline{BC}` 和 :math:`\overline{AC}` 上与该圆相切的点。然后 :math:`\overline{OD} \perp \overline{AB}`、:math:`\overline{OE} \perp \overline{BC}` 和 :math:`\overline{OF} \perp \overline{AC}`。因此，:math:`\triangle\,OAD` 和 :math:`\triangle\,OAF` 是等价三角形，因为它们是直角三角形，斜边 :math:`\overline{OA}` 相同，且对应直角边 :math:`\overline{OD}` 和 :math:`\overline{OF}` 的长度均为 $r$。因此，:math:`\angle\,OAD =\angle\,OAF`，这意味着 :math:`\overline{OA}` 平分角 $A$。类似地， :math:`\overline{OB}` 平分 $B$，:math:`\overline{OC}` 平分 $C$。由此可知：

.. note::

    对于任何三角形，其内切圆的圆心是角平分线的交点。

我们将使用图 :ref:`2.5.6 <fig:inscribed>` 来求内切圆的半径 $r$。由于 :math:`\overline{OA}` 平分 $A$，可知 :math:`\tan\;\frac{1}{2}A = \frac{r}{AD}`，因此 :math:`r = AD \,\cdot\, \tan\;\frac{1}{2}A`。现在，:math:`\triangle\,OAD` 和 :math:`\triangle\,OAF` 是等价三角形，因此 $AD = AF$。类似地，$DB = EB$ 且 $FC = CE$。因此，如果我们令 :math:`s=\frac{1}{2}(a+b+c)`，则
...
因此，:math:`r = (s-a)\,\tan\;\frac{1}{2}A`。对于角 $B$ 和 $C$，类似的推导可得：

.. _thm:inscribedradius:

**定理 2.10.** 对于任意三角形 :math:`\triangle ABC`，令 :math:`s = \frac{1}{2}(a+b+c)`。则其内切圆半径 $r$ 为
...
我们还可以从图 :ref:`2.5.6 <fig:inscribed>` 中看到，三角形 :math:`\triangle\,AOB` 的面积为
...
类似地，:math:`\text{Area}(\triangle\,BOC) = \frac{1}{2}\,a\,r` 和 :math:`\text{Area}(\triangle\,AOC) = \frac{1}{2}\,b\,r`。因此，:math:`\triangle ABC` 的面积 $K$ 为
...
**定理 2.11.** 对于任意三角形 :math:`\triangle ABC`，设 :math:`s = \frac{1}{2}(a+b+c)`。则其内切圆的半径 $r$ 为
...
回想一下几何学中如何平分一个角：用圆规以顶点为中心画一条圆弧，该圆弧与角的两边相交于两点。在这两点处，用圆规画一条半径相同的圆弧，该圆弧足够长，使两条圆弧相交于一点，如图 :ref:`2.5.7 <fig:angbisect>` 所示。过该点和顶点的线段就是角的平分线。对于三角形的内切圆，只需要 *两条* 角平分线；它们的交点就是圆心。
...
.. toggle::
    ...
    **Theorem 2.10.** For any triangle :math:`\triangle ABC`, let :math:`s = \frac{1}{2}(a+b+c)`. Then the radius $r$ of its inscribed circle is
    ...
    .. math::
        ...
        \begin{align*}
        K ~&=~ \text{Area}(\triangle\,AOB) ~+~\text{Area}(\triangle\,BOC) ~+~ \text{Area}(\triangle\,AOC)\,b\,r\\
        ~&=~ \tfrac{1}{2}\,c\,r ~+~ \tfrac{1}{2}\,a\,r ~+~ \tfrac{1}{2}\,b\,r\\
        ~&=~ \tfrac{1}{2}\,(a+b+c)\,b\,r\\ ~&=~ sr ~,~\,b\,r\\
        ~&\text{so by Heron's formula we get}\\
        r ~&=~ \frac{K}{s} \,b\,r\\
        ~&=~ \frac{\sqrt{s\,(s-a)\,(s-b)\,(s-c)}}{s} \,b\,r\\
        ~&=~ \sqrt{\frac{s\,(s-a)\,(s-b)\,(s-c)}{s^2}} \,b\,r\\
        ~&=~ \sqrt{\frac{(s-a)\,(s-b)\,(s-c)}{s}} ~~.
        \end{align*}
...
.. admonition:: Example 2.19
    ...
    根据 2.2 节中例题 :ref:`2.6 <exmp:case4cosine>` 求三角形 :math:`\triangle ABC` 的内切圆半径 $r$：$a = 2$，$b = 3$，$c = 4$。画出圆。
    ...
    **解：** 利用定理 :ref:`2.11 <thm:inscribedarea>` 和 :math:`s = \frac{1}{2}(a+b+c) = \frac{1}{2}(2+3+4) = \frac{9}{2}`，可得

    .. math::

        r ~&=~ \sqrt{\frac{(s-a)\,(s-b)\,(s-c)}{s}} \\
          ~&=~ \sqrt{\frac{\left(\frac{9}{2}-2\right)\,\left(\frac{9}{2}-3\right)\,\left(\frac{9}{2}- 4\right)}{\frac{9}{2}}} \\
          ~&=~ \sqrt{\frac{5}{12}}~。

    图 :ref:`2.5.8 <fig:inscrib234>` 展示了如何画内切圆：画出 $A$ 和 $B$ 的角平分线，然后在它们的交点处用圆规画一个半径为 :math:`r = \sqrt{5/12} \approx 0.645` 的圆。
...
练习 1-6：求三角形 :math:`\triangle ABC` 的外接圆和内接圆的半径 $R$ 和 $r$。
...
在练习 7 和 8 中，使用尺子和圆规（或计算机软件）准确绘制三角形 :math:`\triangle ABC` 及其外接圆和内接圆。

7. $a = 2$ 英寸，$b = 4$ 英寸，$c = 5$ 英寸
8. $a = 5$ 英寸，$b = 6$ 英寸，$c = 7$ 英寸

9. 对于任意三角形 :math:`\triangle ABC`，令 $s = \frac{1}{2}(a+b+c)$。证明
...
10. 证明：对于任意三角形 :math:`\triangle ABC`，其外接圆半径 $R$ 为

    .. math::
    
        R ~=~ \frac{abc}{\sqrt{(a+b+c)\,(b+c-a)\,(a-b+c)\,(a+b-c)}} ~~。

11. 证明：对于任意三角形 :math:`\triangle ABC`，其外接圆半径 $R$ 和内切圆半径 $r$ 满足以下关系：

    .. math::
    
        rR ~=~ \frac{abc}{2\,(a+b+c)} ~~。

12. 设 :math:`\triangle ABC` 为一个等边三角形，其边长为 $a$。

13. 求 :math:`\triangle ABC` 外接圆半径 $R$ 的准确值。
14. 求 :math:`\triangle ABC` 内切圆半径 $r$ 的准确值。
15. $R$ 比 $r$ 大多少？
16. 证明 :math:`\triangle ABC` 的外接圆和内切圆同心。
17. 设 :math:`\triangle ABC` 为直角三角形，且 :math:`C=90^\circ`。证明 :math:`\;\tan\;\tfrac{1}{2}A = \sqrt{\frac{c-b}{c+b}}~`。
